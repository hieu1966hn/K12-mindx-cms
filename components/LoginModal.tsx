    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-md flex items-center justify-center z-50 animate-fade-in" onMouseDown={onClose}>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-md m-4 relative animate-fade-in-up" onMouseDown={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleLogin}>
          {/* Trường nhập email */}
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="email">{UI_STRINGS.username}</label>
            <input
              id="email"
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 focus:border-[#E31F26] focus:ring-0 outline-none transition"
              required
          ...
          <button
            type="submit"
            disabled={isLoggingIn}
            className="w-full bg-[#E31F26] text-white font-bold py-2 px-4 rounded-md hover:bg-red-700 transition-colors duration-300 focus:outline-none focus:ring-0 disabled:bg-red-400 disabled:cursor-not-allowed shadow-lg shadow-red-500/20"
          >
