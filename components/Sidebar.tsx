...
const SidebarItem: React.FC<SidebarItemProps> = memo(({ onClick, href, icon: Icon, children, isSelected = false, tooltip, isCollapsed }) => {
    const commonClasses = "flex items-center w-full p-3 rounded-lg text-left text-base font-semibold transition-all duration-200 ease-in-out gap-4";
    const selectedClasses = "bg-[#E31F26] text-white shadow-md shadow-red-500/30";
    const unselectedClasses = "text-gray-600 dark:text-gray-300 hover:bg-black/5 dark:hover:bg-white/10";

    const content = (
      <>
        <Icon size={22} className="flex-shrink-0" />
        <span className={`flex-1 whitespace-nowrap overflow-hidden transition-opacity duration-200 ${isCollapsed ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          {children}
        </span>
      </>
    );
...
export const Sidebar: React.FC<SidebarProps> = ({ isCollapsed, onToggle }) => {
  ...
  const { data, selectedPathId, setSelectedPathId, setSelectedCourseId, currentUser, logout } = context;
  ...
  const handlePathClick = useCallback((pathId: string | null) => {
    ...
    setSelectedCourseId(null); // Reset lựa chọn khóa học khi đổi lộ trình.
    // Trên di động, tự động đóng sidebar sau khi chọn.
    if (window.innerWidth < 768) { // md breakpoint
        onToggle();
    }
  }, [setSelectedPathId, setSelectedCourseId, onToggle]);
