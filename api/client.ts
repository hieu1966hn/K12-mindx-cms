...
export const fetchCmsData = async (): Promise<CmsData> => {
    ...
    pathSnap.forEach(s => cmsData.entities.learningPaths[s.id] = { ...(s.data() as Omit<LearningPath, 'courses'|'documents'>), courseIds:[], documentIds:[] });
    courseSnap.forEach(s => cmsData.entities.courses[s.id] = { ...(s.data() as Omit<Course, 'levels'|'documents'>), pathId: s.data().pathId, levelIds:[], documentIds:[] });
    levelSnap.forEach(s => cmsData.entities.levels[s.id] = { ...(s.data() as Omit<Level, 'documents'>), courseId: s.data().courseId, documentIds:[] });
    ...
    Object.values(cmsData.entities.documents).forEach(doc => {
        if(doc.levelId && doc.courseId && cmsData.entities.levels[doc.levelId]) {
            cmsData.entities.levels[doc.levelId].documentIds.push(doc.id);
        } else if (doc.courseId && cmsData.entities.courses[doc.courseId]) {
            cmsData.entities.courses[doc.courseId].documentIds.push(doc.id);
        } else if (doc.pathId && cmsData.entities.learningPaths[doc.pathId]) {
            cmsData.entities.learningPaths[doc.pathId].documentIds.push(doc.id);
        }
    });

    cmsData.root = Object.keys(cmsData.entities.learningPaths);
    
    // Sắp xếp documents theo thứ tự đã lưu trong DB
    Object.values(cmsData.entities.learningPaths).forEach(p => {
        if (p.documentIds) p.documentIds.sort((a,b) => (cmsData.entities.documents[a] && cmsData.entities.documents[b]) ? (p.documentIds.indexOf(a) - p.documentIds.indexOf(b)) : 0);
    });
    Object.values(cmsData.entities.courses).forEach(c => {
        if (c.documentIds) c.documentIds.sort((a,b) => (cmsData.entities.documents[a] && cmsData.entities.documents[b]) ? (c.documentIds.indexOf(a) - c.documentIds.indexOf(b)) : 0);
    });
    Object.values(cmsData.entities.levels).forEach(l => {
        if (l.documentIds) l.documentIds.sort((a,b) => (cmsData.entities.documents[a] && cmsData.entities.documents[b]) ? (l.documentIds.indexOf(a) - l.documentIds.indexOf(b)) : 0);
    });
